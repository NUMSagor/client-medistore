...
const openEdit = (m: Medicine) => {
    setForm({
        ...
        manufacturer:    m.manufacturer,   // ← fix
        description:     m.description,    // ← fix
        price:           String(m.price),
        discountPercent: String(m.discountPercent),
        stock:           String(m.stock),
        categoryId:      m.category?.id,   // ← fix
        imageUrl:        m.imageUrl ?? '',
    });
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openAdd}
          className="flex items-center gap-2 bg-linear-to-r from-indigo-600 to-pink-700 text-white text-sm font-semibold px-4 py-2 rounded-lg hover:opacity-90 transition-opacity">
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-xl">
            ...
            <div className="p-6 flex flex-col gap-4">
              ...
              <button onClick={handleSave} disabled={saving}
                className="w-full bg-linear-to-r from-indigo-600 to-pink-700 text-white font-semibold py-2.5 rounded-lg hover:opacity-90 disabled:opacity-60 transition-opacity">
